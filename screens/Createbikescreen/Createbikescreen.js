...
import { View, Text, TextInput, TouchableHighlight, TouchableOpacity, TouchableNativeFeedback } from 'react-native';
...
function Createbikescreen(props) {
    ...
    const saveNewBike = async() => {
        if ((state.name || state.model || state.color || state.accesories) === '') {
            ...
            await firebase.db.collection('Bicicleta').add({
                name: state.name,
                model: state.model,
                color: state.color,
    ...
    return (
        <View style={styles.container}>
            ...
            <TouchableOpacity style={styles.button} onPress={() => saveNewBike()}>
